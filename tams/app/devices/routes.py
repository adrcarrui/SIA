    ...
    if q:
        ...
        query = query.filter(
            or_(
                ...
                models.Device.barcode.ilike(like),
    ...
    try:
        ...
        linked=(
            db.query(func.count(Assignment.id))
                .filter(Assignment.device_id == d.id)
                .scalar()
        )

        if linked and linked > 0:
            flash(
                f"Cannot delete '{d.name}' because it is linked to {linked} course assignments().\n"
                f"Close/remove the assignment(s) first.",
                "warning",
            )
            return redirect(url_for("devices.index"))
