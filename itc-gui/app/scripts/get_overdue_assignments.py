...
def get_cards_vs_trainees_alerts(db, managed_by: str | None = None):
    ...
    Cuenta SOLO assignments activos (released_at NULL, status active)
    y SOLO devices cuyo AssetType.show_in_calendar=True.

    Si managed_by != None, limita a AssetType.managed_by_department == managed_by.
    """
    today = date.today()

    assigned_count = func.sum(
        case(
            (AssetType.show_in_calendar.is_(True), 1),
            else_=0,
        )
    )
    ...
    # Scope por departamento (opcional)
    if managed_by:
        q = q.filter(AssetType.managed_by_department == managed_by)

    rows = q.group_by(Course.id).all()
